    ...
    def forward(self, coeffs, render=True, texture=True):
        ...
        if render:
            ...
            if texture:
                ...
                face_color = (face_color * 0 + 200).to(torch.float32)

                rendered_img = torch.zeros(size=(0, self.img_size, self.img_size, 4)).to(self.device)
                chunk_size = 256
                for i in range(0, vs_t.size(0), chunk_size):
                    vs_chunk = vs_t[i:i + chunk_size]
                    tri_chunk = self.tri.repeat(vs_chunk.size(0), 1, 1)
                    color_chunk = face_color[i:i + chunk_size]
                    tex_chunk = TexturesVertex(color_chunk)
                    mesh_chunk = Meshes(verts=vs_chunk, faces=tri_chunk, textures=tex_chunk)
                    out = self.renderer.sha_renderer(mesh_chunk)
                    rendered_img = torch.cat((rendered_img, out), dim=0)
